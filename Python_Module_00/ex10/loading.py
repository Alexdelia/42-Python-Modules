...
def ft_progress(
    ...
    for i, item in enumerate(listy):
        ...
        print(
            f"\r\033[1;2;34mETA:\033[0m \033[1;3;34m{eta:.2f}s\033[0m",
            f"\t\033[1m[\033[32m{percent:3.0f}%\033[0m\033[1m]",
            f"[\033[35m{fill * perw}{[tip,fill][i == size - 1]}\
\033[0m{' ' * (w - perw)}\033[1m]",
            f"\033[3m{i + 1}\033[0m\033[1m/\033[2;3m{size}\033[0m",
            f"\t\033[1m| \033[2;33melapsed time\
 \033[0m\033[1;3;33m{elapsed:.2f}s",
            end="\033[0m"
        )
        yield item
...
if __name__ == "__main__":
    print(
        f"{__file__} only contain the function",
        f"ft_progress:\n{ft_progress.__doc__}"
    )
