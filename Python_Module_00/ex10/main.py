...
def test(
    ...
    for item in ft_progress(listy):
        ...
        sleep(sleep_time)
    print(
        "\n>>\033[1;3;32m", [f"{ret}", f"{ret:e}"][ret > 100000000],
        end="\033[0m\n\n"
    )
...
def test_w_fill_tip(
    ...
    """
    This function tests the ft_progress\
function with a custom width, fill and tip.
    """
    print(
        f">> \033[1;3;36mTEST {name}",
        f"(width: {w}, fill '{fill}', tip: '{tip}')\033[0m"
    )
    ret = 0
...
test("10 ([])", [], lambda x: x, 1)  # type: ignore
try:
    test("11 (None)", None, lambda x: x, 1)  # type: ignore
except TypeError:
    print(">> \033[1;3;31mTypeError\t[KO]\033[0m\n")
test("12 (str)", "Hello World!", lambda x: 0, 0.1)  # type: ignore
